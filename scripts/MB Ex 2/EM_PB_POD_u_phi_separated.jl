...
function get_symbolic_res_and_jac(dΩ)
    diffstrat = "autodiff"
    soltype = "monolithic"
    modmec = Yeoh(C₁ = 0.8*0.0693e6, C₂ = -8.88e2*0.8, C₃ = 0.8*16.7, κ = 0.0693e8)
...
function runs()
    DEIM_Matrices = load("scripts/MB Ex 2/DEIM_matrices_u_phi_sep.jld2")
    ...
    x0 = zeros(Float64,2*m_k)
    K_T_DEIM_ = Matrix{Float64}(undef, 2*m_k, n)
    K_T_DEIM = Matrix{Float64}(undef, n, n)
    K_T_DEIM_red_ = Matrix{Float64}(undef, n, 2*m_k)
    K_T_DEIM_red = Matrix{Float64}(undef, 2*m_k, 2*m_k)
    x_list = [copy(x0)]
    x_ = zeros(Float64,2*m_k)
    t0 = time()
    time_step = []
    x_list = []
    b_list = []
...
function Red_Error(x_list)
    ...
    model, Ω, dΩ =  get_trian_and_measure()
    _, m_k = size(𝛟_u)
    file_name = "data/csv/EM_PB_test/MaterialModel2/x_.csv"
    ...
    mkpath("data/sims/EM_PB_test/Red/Result_withError_POD-DEIM_u_phi_sep")
    pvd = paraview_collection("data/sims/EM_PB_test/Red/Result_withError_POD-DEIM_u_phi_sep" * "/Results", append=false)
    writevtk(model, "data/sims/EM_PB_test/Red/Result_withError_POD-DEIM_u_phi_sep" * "/DiscreteModel")
    dirichletbc = get_DirichletBC(0.0) #lastindex(x_list)))
    fe_spaces = get_fe_spaces(model,dirichletbc)
    U_ = fe_spaces.U
    # _x = _X[:,i]
    # maximum(abs.(x[[1:u_dofs...]]))
    for i in 1:75 #lastindex(x_list)
        ...
        pvd[Λ_] = createvtk(
            Ω,
            "data/sims/EM_PB_test/Red/Result_withError_POD-DEIM_u_phi_sep"* "/_Λ_" * Λstring * "_TIME_$Λ_" * ".vtu",

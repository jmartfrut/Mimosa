...
function get_symbolic_res_and_jac(dΩ)
    diffstrat = "autodiff"
    soltype = "monolithic"
    modmec = Yeoh(C₁ = 1.0*0.0693e6, C₂ = -8.88e2*1.1, C₃ = 1.0*16.7, κ = 0.0693e8)
...
function Red_Error(x_list)
    ...
    _, m = size(𝛟_u)
    _, k = size(𝛟_𝜑)
    file_name = "data/csv/EM_CM_test_2/MaterialModel1/x_.csv"
    ...
    mkpath("data/sims/EM_CM_test_2/Red/Result_withError_POD-DEIM_u_phi_sep_3")
    pvd = paraview_collection("data/sims/EM_CM_test_2/Red/Result_withError_POD-DEIM_u_phi_sep_3" * "/Results", append=false)
    writevtk(model, "data/sims/EM_CM_test_2/Red/Result_withError_POD-DEIM_u_phi_sep_3" * "/DiscreteModel")
    ...
    for i in 1:lastindex(x_list)
        ...
        pvd[Λ_] = createvtk(
            Ω,
            "data/sims/EM_CM_test_2/Red/Result_withError_POD-DEIM_u_phi_sep_3"* "/_Λ_" * Λstring * "_TIME_$Λ_" * ".vtu",

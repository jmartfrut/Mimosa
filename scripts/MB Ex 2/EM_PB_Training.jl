...
function collect_data()
    ...
    for i in 1:3
        ...
        mkpath("data/csv/EM_PB_test_V2/MaterialModel$f")
        CSV.write("data/csv/EM_PB_test_V2/MaterialModel$f/x_.csv",df_x)
        CSV.write("data/csv/EM_PB_test_V2/MaterialModel$f/b_.csv",df_b)
        CSV.write("data/csv/EM_PB_test_V2/MaterialModel$f/R_nl_.csv",df_R_nl)
    end
end
...
function compute_VTK()
    ...
    fe_spaces = get_fe_spaces(model,dirichletbc)
    file_name = "data/csv/EM_PB_test/MaterialModel0/x_.csv"
    _X = CSV.File(file_name) |> Tables.matrix
    mkpath("data/sims/EM_PB_test/MaterialModel0")
    pvd = paraview_collection("data/sims/EM_PB_test/MaterialModel0"* "/Results", append=false)
    writevtk(model, "data/sims/EM_PB_test/MaterialModel0"* "/DiscreteModel")
    i = 1
    for _x in eachcol(_X)
        ...
        pvd[Λ_] = createvtk(
            Ω,
            "data/sims/EM_PB_test/MaterialModel0" * "/_Λ_" * Λstring * "_TIME_$Λ_" * ".vtu",
